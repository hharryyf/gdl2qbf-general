...
for config in dir:
    for inst in instance:
        ...
        for solver in ['depqbf', 'caqe']:
            ...
            try:
                subprocess.run([f'{solver}', f'instances/{config}/bloq_{inst}.qdimacs'], start_new_session=True, timeout=1200)
            except subprocess.TimeoutExpired as e:
                print('TIMEOUT')
            end1 = time.time()
            os.system('sleep 0.5')
            f = open('result.txt', 'a')
            if end - start + end1 - start1 > 1200:
                print(f'config: {config} | instance: {inst}  | solver: {solver} | time: > 1200', file=f)
            else:
