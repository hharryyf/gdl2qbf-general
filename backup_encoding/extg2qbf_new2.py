...
def base_encoding(inputfile, name, adv, horizon, turn):
    ...
    for player in turn:
        for i in range(1, horizon + 1):
            if i in turn[player]: 
                print(f'does({player}, A, {i}) :- legal({player}, A, {i}), not terminated({i}).', file=f)
            else:
                print('{' + f'does({player}, A, {i}) : input({player}, A)' + '}' + f':- not terminated({i}).', file=f)
...
if __name__ == '__main__':
    ...
    if noop == True:
        turnmap[name] = set(get_o_noop(path, horizon, 2, name))
        for o in adv:
            turnmap[o] = set(get_o_noop(path, horizon, 2, o))

    gamma = base_encoding(path, name, adv, horizon, turnmap)
    ...
    for o in adv:
        ...
        model_adverse(o, gamma[o], turnmap[o], horizon, fg)
        fg = 'a'
