...
The QBF instances answer the question: can a player get 100 points within n steps regardless of the actions of all the other players. The GDL descriptions of the games are available at: https://github.com/hharryyf/gdl2qbf-general/tree/main/SinglePlayer/GameDescriptions. The QBF instances are generated by the toolchain: GDL -> temproal extented GDL -> QASP -> QBF. The framework is formally introduced in our AAMAS 2024 paper: https://www.ifaamas.org/Proceedings/aamas2024/pdfs/p807.pdf Instance generator is available in the root directory: https://github.com/hharryyf/gdl2qbf-general/


## Data Description

```naive_encoding.zip``` (or the ```GB/``` directory) contains instances generated with the baseline encoding (Def. 6 in the paper).

```dependency_based_encoding.zip``` (or the ```GD/``` directory) contains instances generated with the dependency-based encoding (Def. 8 in the paper). These instances perform quantifier shifting based on the atom dependency graph of the GDL description. Correctness can be justified by the splitting theorem.
